  ...
  return (
    <Box sx={{ maxWidth: 700 }}>
      ...
      <Section title="Weekly Summary">
        ...
        <B>Live Dashboard</B>
        <P>The <strong>Live Dashboard</strong> link in the sidebar opens a full interactive dashboard (at /report) that reads live straight from the database. Pick a rolling preset (7d / 30d / 90d), a calendar preset (WTD / MTD / QTD / YTD — this week, month, quarter, or year to date), or a custom date range. The active window is always spelled out as absolute dates under the title (e.g. "Showing Jun 1 → Jun 24 · 24 days"), and notes when a preset was capped at the start of your history. Every stat compares against the immediately preceding equal-length window and carries a mini weekly-trend sparkline. A row of auto-detected callouts (most productive day, busiest week, after-hours share, days out of office, and a backlog-pace read — growing, steady, or "clears in ~N weeks" at the current rate) sits under the summary — deliberately limited to facts that aren't already one of the stat cards. It shows weekly throughput, a maker-vs-manager split with a capacity bar (what % of your working hours is maker/focus time vs manager/meeting time), meeting fragmentation (how many meetings chop up each week, with the average meeting length), meeting load by weekday (which day to protect), the type mix, a "when work happens" grid (a weekday × time-of-day crosstab of completions), turnaround by type (median created→done days per work type, so you see what actually drags), interruptions vs output (average completions on interrupted days vs calm days), project momentum (which projects rose or fell vs the prior window), and a per-day activity heatmap — click any day to see the items completed that day (amber ring = after-hours, dashed teal = out of office). The context-switches bar chart is also clickable — click any day's bar to see exactly what you switched between that day — a single chronological timeline that interleaves everything in time order regardless of kind: each work context (the task you picked up, tagged with its project), each meeting, and each non-task yank (with its category and any note you jotted). Tallies logged without a timestamp sort to the end.</P>
        <B>How you work (secondary stats)</B>
        <P>Under the headline row is a second row that looks at <em>how</em> you work, not just how much: <strong>Net flow</strong> (items created vs completed in the window — a positive number means the backlog grew, negative means it shrank), <strong>Deep-work days</strong> (workdays with zero meetings, out of the total workdays in range, plus how many were "heavy" with 4+ meetings), <strong>Typical turnaround</strong> (median days from an item being created to being marked done, with the average and sample size), <strong>Active streak</strong> (consecutive days with completed work — current and longest — plus total active days), <strong>Open items (now)</strong> (a live count of genuinely-open work — Active, Inbox, or Waiting), and <strong>Project focus</strong> (what share of output your top three projects account for, out of how many total — a high number means concentrated effort, a low one means you're spread thin). The <strong>After-hours items</strong> card also breaks its count into evening vs weekend, a cleaner boundary-health read than one lumped figure.</P>
        <B>Self-directed vs interruptions</B>
        <P>The context-switch total is also broken into <strong>self-directed</strong> churn (you changed what you were working on — a new task or meeting) versus <strong>interruptions</strong> (non-task yanks you logged as tallies). A dedicated weekly stacked chart shows the split over time — a rising orange (interruptions) band means a noisier week.</P>
        <B>Open work / carryover</B>
        <P>A full-width panel lists everything currently open — items in <strong>Active</strong>, <strong>Inbox</strong>, or <strong>Waiting</strong> — with its age since creation, oldest first, colored by how long it's been sitting (amber past 14 days, red past 30). Parked statuses (Later, Scheduled) are excluded — they're deliberate future work, not carryover. Status chips show the breakdown across those open statuses, and a second row of age chips buckets open items by how long they've been sitting (0–7d / 8–14d / 15–30d / 30d+). This is a live snapshot of what's piling up — it is <em>not</em> bound to the selected date range, unlike every other panel.</P>
        <B>Collaboration</B>
        <P>A panel contrasts <strong>PRs reviewed</strong> (reviewing teammates' work — helping the team) against <strong>PRs merged</strong> (shipping your own), with the split as a bar and a review-to-author ratio. A high ratio means you're spending more of your PR time unblocking others than shipping yourself.</P>
        <B>A note on focus hours</B>
        <P>Focus hours are <em>estimated</em>, not measured — they're your work-hours window minus meetings and out-of-office time. The card is labelled "Focus hours (est.)" to make that clear; it represents available focus capacity, not time you provably spent heads-down.</P>
      </Section>
